...
class Instance:
    def __init__(self, width, height, succ_probs, costs):
        self.width = width
        self.height = height
        ...
        assert len(costs) == self.height
        for cost in costs:
            assert len(cost) == self.width
        ...
        self.costs =  { self.get_state(x,y) : costs[y][x] for y in range(self.height) for x in range(self.width) }
    ...
    def __str__(self):
        ...
        for y in reversed(range(self.height)):
            res += "[ " + "  ".join("{:.0f}".format(self.costs[self.get_state(x,y)])
                for x in range(self.width)) + " ]\n"
...
def get_example_instance():
    ...
    return Instance(4, 5, probs, costs)
